...
export default function MyEvents() {
  ...
  const t = {
    ...
    loading: isArabic ? 'جارٍ التحميل...' : isKurdish ? 'چاوەڕوانبە...' : 'Loading...',
  ...
  return (
    <div dir={isRtl ? 'rtl' : 'ltr'} className="max-w-6xl mx-auto px-4 py-8 overflow-hidden font-sans">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          {events.map((event) => (
            <motion.div
              ...
              key={event.id}
              className="bg-white dark:bg-[#13091f] border border-slate-200/80 dark:border-[#2a1745] rounded-3xl overflow-hidden shadow-sm hover:border-purple-400 dark:hover:border-purple-500/50 flex flex-col justify-between group transition-all duration-300"
            >
              <div className="relative h-48 overflow-hidden bg-slate-100 dark:bg-slate-900">
                ...
                />
                <span className={`absolute top-3 ${isRtl ? 'right-3' : 'left-3'} text-[10px] font-bold px-3 py-1 rounded-xl uppercase shadow-sm backdrop-blur-md border border-white/10 ${
                  event.status === 'published' ? 'bg-emerald-500/80 text-white' :
                  event.status === 'draft' ? 'bg-amber-500/80 text-white' :
                  event.status === 'cancelled' ? 'bg-rose-500/80 text-white' : 'bg-slate-500/80 text-white'
                }`}>
                  {event.status}
                </span>
              </div>
        ...
        <div className="bg-white dark:bg-[#13091f] border border-slate-200/80 dark:border-[#2a1745] rounded-3xl p-10 text-center max-w-md mx-auto space-y-4 my-12 shadow-sm">
            ...
            >
              ...
              <form onSubmit={handleUpdate} className="space-y-4">
                ...
                <div>
                  ...
                  <input
                    type="datetime-local" required value={editForm.date_time}
